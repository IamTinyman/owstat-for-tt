...
def create_server(config: APIConfig) -> ThreadingHTTPServer:
    ...
    class OverstatsRequestHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            if path == "/api/v2/ui/bootstrap":
                try:
                    from overstats.src.http_server.registry import get_http_ui_bootstrap_payload
                except ModuleNotFoundError:
                    from src.http_server.registry import get_http_ui_bootstrap_payload
                self._send_json(HTTPStatus.OK, get_http_ui_bootstrap_payload())
                return
